...
<script>
  ...
  import * as re_review from '../../network/manage/review_list.js'
    ...
    methods: {
      ...
      onSearch(val) {
        ...
        this.searchTableData = this.oldData.filter(item => {
          return item.username.match(this.searchList.username) && item.group_name.match(this.searchList.group_name)
        })
      ...
      getList() {
        ...
        re_review.getReviewList(this.userInfoList.uid).then(res => {
      ...
      del(row) {
        console.log("DEL")
        this.$confirm('此操作将删除此动态, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          })
          .then(() => {
            re_review.delReview(row.review_id).then(res => {
              console.log(res.data)
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      deleteMore() {
        ...
        this.multipleSelection.forEach(item => {
          deleteList.push(item.review_id)
        })
        this.$confirm('此操作将永久删除所选评价, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          })
          .then(() => {
            re_view.deleteMore(deleteList).then(res => {
              console.log(res.data)
              this.$message({
                type: 'success',
                message: '删除成功!'
