<template>
  <div style="padding: 30px;">
      ...
      <Table :tableData="filterTableData" :tableLabel="tableLabel" :config="config" @changePage="showData" :opt="opt"
        @handleSelectionChange="handleSelectionChange" @del="del" @changeTableSort="changeTableSort"></Table>
    </div>
</template>
...
<script>
  ...
  import * as re_topic from '../../network/manage/topic_list.js'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        re_topic.getTopicList().then(res => {
      ...
      del(row) {
        console.log("DEL")
        this.$confirm('此操作将永久删除此动态, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          })
          .then(() => {
            re_topic.delTopic(row.id).then(res => {
                console.log(res.data)
                this.$message({
                  type: 'success',
                  message: '删除成功!'
      ...
      deleteMore() {
        ...
        this.$confirm('此操作将批量永久删除所选动态, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          })
          .then(() => {
            re_topic.deleteMore(deleteList).then(res => {
                console.log(res.data)
                this.$message({
                  type: 'success',
                  message: '删除成功!'
